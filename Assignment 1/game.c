...
void displayMap(char** mapArray, int mapRow, int mapCol, int visibility, int* playerRowLoc, int* playerColLoc)
{
    ...
    if (visibility == 0)
    { /*If we visibility is 0 then display the full 2d array*/
        leftBoundary = 0;
    ...
    {
        if (*playerColLoc -  visibility < 0)
        {/*If the visibility range exceeds the boundary assign 0, else assign the respective value*/
            leftBoundary = 0;
        }
        else
        {
            leftBoundary = *playerColLoc - visibility;
        }

        if (*playerColLoc +  visibility >= mapCol)
        {/*If the visibility range exceeds the boundary assign 0, else assign the respective value*/
            rightBoundary = mapCol - 1;
        }
        else
        {
            rightBoundary = *playerColLoc + visibility;
        }

        if (*playerRowLoc + visibility >= mapRow)
        {/*If the visibility range exceeds the boundary assign 0, else assign the respective value*/
            bottomBoundary = mapRow - 1;
        }
        else
        {
            bottomBoundary = *playerRowLoc + visibility;
        }

        if (*playerRowLoc - visibility < 0)
        {/*If the visibility range exceeds the boundary assign 0, else assign the respective value*/
            topBoundary = 0;
        }
        else
        {
            topBoundary = *playerRowLoc - visibility;
        }
    }
...
void movePlayer(char*** mapArray, char** player , char input, int mapCol, int mapRow, int* playerRowLoc, 
